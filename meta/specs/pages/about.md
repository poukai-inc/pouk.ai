...
- **Aspect ratio**: portrait orientation, **3:4** (the editorial-portrait standard). Source artifact is head-and-shoulders single subject — 3:4 frames the subject without forcing a tight head crop. If the source artifact has different ratio, Arian crops at production time to 3:4 before handoff. Designer composition revision may revise this if a different aspect serves Direction A's composition better; PM default is 3:4.
- **Source resolution**: ≥ 1600px on the long axis. Engineer downsamples for served variants.
...
- **JSON-LD**: standalone `schema.org/Person` schema only (A15b v1, holds in v2). Fields: `name` (`"Arian Zargaran"`), `jobTitle` (`"Founder, pouk.ai"`), `url` (`"https://pouk.ai/about"`). No `worksFor`. No `sameAs`. No additional fields surfacing the portrait or its provenance — structured data restraint extends to the visual asset.
